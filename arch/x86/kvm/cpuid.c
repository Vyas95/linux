...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	// Report the Total Time Spend in Exit :BHARATH
		...
		eax=0;
		printk("Bharath:->TotalTime: %lld\n", temp_totalTime);
	}


	// Report the number of exits/Reason :Bharath
	else if(eax  ==  0x4ffffffd){
		if(ecx >= 0 && ecx < 70){
			eax = atomic_read(&number_of_exits_occured[ecx]);
			printk("Bharath:->Exits/Reason:  %d\n", eax);
		}	
	}

	// Report the Time Spend in Exit/Reason: Bharath
		...
		if(ecx >= 0 && ecx < 70){
			...
			eax=0;
			printk("Bharath:->Time/Exit ecx=%d:  %lld\n", ecx_temp,temp_time);
		}	
	}
	else {
